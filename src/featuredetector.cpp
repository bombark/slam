...
int FeatureDetector::getFeatures(std::vector<Feature> *featVec, double* structCompass, double curPhi, std::vector<float>& readings){
...
int FeatureDetector::fitLineSegments (
  ...
  for (int r=0; r<readings.size(); r++){
    if (readings[r] > MAX_DIST) continue;
