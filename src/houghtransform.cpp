...
int HoughTransform::getLines(std::vector<float>& readings, std::vector<struct houghLine> *lines){
    //perform hough transform
    this->performHoughTransform(readings);
    // this->sendHoughToImage("hough.pgm");
...
void HoughTransform::performHoughTransform(std::vector<float>& readings){
    int radius;
    double x, y;
  
    double diff = 3.141592 / 180.0;
    for (int i=0; i<readings.size(); i++) {
        ...
        double angle = i * diff;
        x = readings[i] * sin(angle) * 2.0;
        y = readings[i] * cos(angle) * 2.0;
// cout << angle << " " << x << " " << y << endl;
